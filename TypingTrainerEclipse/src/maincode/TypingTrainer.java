...
public class TypingTrainer {
	...
	public static String[] generateQuestionStringArray() {
		...
		while(currentQuestionNo<numberOfQuestions)
		{
			...
			while(currentSlotAssigned==false)
			{
				...
				//CREATE RANDOM VALUE
				randCharNo = rand.nextInt(numberOfQuestions);//ATTENTION RANDOM EXCLUDES MAX.VALUE
	...
	public static void printArray() {
		// PRINT ALL CHARACTERS:
		for (int i = 0; i < numberOfQuestions; i++) {
			System.out.println("Question number "+ i +" points to char number "+
					allQuestionsArrayNo[i]+ " = String: "+allQuestionsArrayString[i]);
		}
	}
	...
	public static void saveTime() {
		...
		for (int i = 0; i < numberOfQuestions; i++) {
			// System.out.printf("Time of %c seconds%.1f%n",allCharsArray[i],timeArray[i]);
		}
		// SMOOTHED VELOCITY
		...
		if (smoothedSignsPerMinute == 0.0) {
			...
			smoothedSignsPerMinute = (smoothedSignsPerMinute * (smoothvalue - 1) + currentSignsPerMinute) / smoothvalue;
		}
	...
	public static void main(String[] args) {
		...
		createCharArray();
		//timeForAllStart = System.nanoTime();// time to solve all questions once
